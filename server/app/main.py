...
# Add the server root to Python path so imports resolve correctly
_server_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _server_root not in sys.path:
    sys.path.insert(0, _server_root)
# Change working directory to server root so relative paths (config/, data/, models/) work
...
# CORS middleware — allow Vite dev server and production origins
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
